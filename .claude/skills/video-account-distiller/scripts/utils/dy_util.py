...
def generateSecretPhoneNum(phone):
    sign = call_js(login_js, 'generateSecretPhoneNum', phone)
    return sign
def generateSecretCode(phone, code):
    sign = call_js(login_js, 'generateSecretCode', phone, code)
    return sign
...
def call_js(context, name, *args):
    original_popen = subprocess.Popen
    original_execjs_popen = execjs_external_runtime.Popen
    subprocess.Popen = partial(original_popen, encoding="utf-8")
    execjs_external_runtime.Popen = partial(original_execjs_popen, encoding="utf-8")
    try:
        return context.call(name, *args)
    finally:
        subprocess.Popen = original_popen
        execjs_external_runtime.Popen = original_execjs_popen
...
def generate_req_sign(e, priK):
    sign = call_js(dy_js, 'get_req_sign', e, priK)
    return sign


# query, data都是拼接字符串
def generate_a_bogus(query, data=""):
    a_bogus = call_js(dy_js, 'get_ab', query, data)
    return a_bogus
...
def generate_signature(room_id, user_unique_id):
    ...
    result = call_js(sign_js, "get_signature", x_ms_stub)
    return result.get("X-Bogus")


# 传递私钥
def generate_ree_key(prik):
    ree_key = call_js(dy_js, 'get_ree_key', prik)
    return ree_key
